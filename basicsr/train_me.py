...
from utils.logger_me import TensorboardLogger
from utils.misc_me import exist, grad_clip, model_load, path_legal_checker
...
def create_dataset(train_dataset_path="../data/wv3/train_wv3.h5",
                    valid_dataset_path="../data/wv3/valid_wv3.h5",
                    dataset_name="wv3",
                    batch_size=16):
...
def train_pipeline(root_path):
    ...
    train_loader, val_loaders = result
    ...
    total_epochs = 40_0000
    for epoch in range(start_epoch, total_epochs + 1):
        for i, (pan, lms, hr, wavelets) in enumerate(train_loader, 1):
            pan, lms, hr, wavelets = map(lambda x: x.cuda(), (pan, lms, hr, wavelets))
            b, c, h, w = lms.shape
            lms = F.interpolate(lms, size=(int(h//2), int(w//2)),mode='bilinear')
            pan = F.interpolate(pan, size=(int(h//2), int(w//2)),mode='bilinear')
            # train_sampler.set_epoch(epoch)
            # prefetcher.reset()
            # train_data = prefetcher.next()
            train_data = {"lq": torch.cat([lms, pan], dim=1), "gt": hr}
            ...
            if current_iter %10000 == 0:
                ...
                with torch.no_grad():
                    for i, (pan, lms, hr, wavelets) in enumerate(val_loaders, 1):
                        ...
                        # prefetcher.reset()
                        # train_data = prefetcher.next()
                        valid_data = {"lq": torch.cat([lms, pan], dim=1), "gt": hr}
                        model.feed_data(valid_data)
                        sr = model.optimize_parameters(current_iter,mode="valid")
                        #sr = sr + lms
                        sr = sr.clip(0, 1)
                        ...
                        hr = torchvision.utils.make_grid(hr, nrow=4, padding=0).cpu()
                        x = torchvision.utils.make_grid(sr, nrow=4, padding=0).detach().cpu()
                        x = x.clip(0, 1)
                        ...
                        fig.savefig(
                            f"../samples/valid_samples/iter_{current_iter}.png",
                            dpi=200,
                            bbox_inches="tight",
                            pad_inches=0,
                        )

                        logger.info("---diffusion result---")
                        logger.info(analysis_d.last_acc)
                    if i != 1:
                        logger.info("---diffusion result---")
                        logger.info(analysis_d.print_str())
