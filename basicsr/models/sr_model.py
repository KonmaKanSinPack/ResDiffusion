...
from diffusion.diffusion_res import extract, make_sqrt_etas_schedule
...
class SRModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.p2_loss_weight_k = 1.0
        self.p2_loss_weight=to_torch(
    ...
    def optimize_parameters(self, current_iter,mode="train"):
        if mode=="valid":
            with torch.no_grad():
                self.net_g.eval()
                out = self.net_g(self.lq, t=self.t, cond=self.cond)
                self.net_g.train()
                return out
        self.optimizer_g.zero_grad()
        self.output = self.net_g(self.lq, t=self.t, cond=self.cond)
        l_total = 0
        ...
        l_total = (
                       l_total * extract(self.p2_loss_weight, self.t, l_total.shape)
               ).mean()
        l_total.backward()
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output = self.net_g_ema(self.lq,t=self.t,cond=self.cond)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.lq,t=self.t,cond=self.cond)
