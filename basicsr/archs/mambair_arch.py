...
from models.sr3_dino import ResnetBlocWithAttn, Swish, Downsample
from models.sr3_dwt import PositionalEncoding
...
class MambaIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = ResnetBlocWithAttn(
            num_in_ch,
            embed_dim,
            cond_dim=num_in_ch + 1,
        ...
        self.down = Downsample(embed_dim)
    ...
    def forward(self, x, t, cond):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            # torch.Size([16, 180, 32, 32])
            # torch.Size([16, 180, 32, 32])
            # torch.Size([16, 64, 32, 32])
            # torch.Size([16, 8, 64, 64])
            x = self.conv_first(x,t,cond[:, : self.lms_channel + self.pan_channel])
            x = self.down(x)
            for layers in self.mid:
                x = layers(x,t)
            x = self.conv_after_body(self.forward_features(x)) + x #核心不同点
