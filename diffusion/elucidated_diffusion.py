...
class ElucidatedDiffusion(nn.Module):
    ...
    def preconditioned_network_forward(
        self, noised_images, sigma, self_cond=None, cond=None, clamp=True
    ):
        ...
        if clamp:
            # g_sr = cond[:, -out.shape[1]:]
            # out = (out + g_sr).clamp(0, 1.0)
            # out = out - g_sr
            out = self.clamp_fn(out)
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, num_sample_steps=None, cond=None, clamp=True):
        # sencond order keun sampling
        ...
        for sigma, sigma_next, gamma in tqdm(
            ...
            model_output = self.preconditioned_network_forward(
                images_hat, sigma_hat, self_cond, cond, clamp=clamp
            )
            ...
            if sigma_next != 0:
                ...
                model_output_next = self.preconditioned_network_forward(
                    images_next, sigma_next, self_cond, cond=cond, clamp=clamp
                )
        ...
        # g_sr = cond[:, -images.shape[1]:]
        # images = (images + g_sr).clamp(0, 1.0)
        # images = images - g_sr

        images = self.clamp_fn(images)

        # return unnormalize_to_zero_to_one(images + cond[:, : images.shape[1]])
        return images + cond[:, :images.shape[1]]

    @torch.no_grad()
    def sample_using_dpmpp(self, batch_size=16, num_sample_steps=None, cond=None):
        """
    ...
    def forward(self, images, cond=None):
        ...
        if self.self_condition and random() < 0.5:
            ...
            with torch.no_grad():
                self_cond = self.preconditioned_network_forward(
                    noised_images, sigmas, cond=cond
                )
                self_cond.detach_()

        denoised = self.preconditioned_network_forward(
            noised_images, sigmas, self_cond, cond
        )
        ...
        return (
            losses.mean(),
            denoised + cond[:, : denoised.shape[1]]
        )

    def set_clamp_fn(self, clamp_fn):
