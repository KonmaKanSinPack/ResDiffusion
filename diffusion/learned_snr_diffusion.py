...
class learned_noise_schedule(nn.Module):
    ...
    def forward(self, x):
        ...
        normed = self.slope * \
            ((x - out_zero) / (out_one - out_zero)) + self.intercept
        # only use a fraction of gradient
...
class ContinuousTimeGaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        learned_noise_schedule_frac_gradient=1.0,
    ...
    def p_sample_loop(self, shape):
        ...
        steps = torch.linspace(
            1.0, 0.0, self.num_sample_steps + 1, device=self.device)
