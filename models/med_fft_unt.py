...
class Conditioning(nn.Module):
    ...
    def forward(self, x, c):
        ...
        if x.shape[-1] != self.ff_parser_attn_map.shape[-1]:
            ff_parser_attn_map = F.interpolate(
                self.ff_parser_attn_map[None, ...],
        ...
        for ind, ((dim_in, dim_out), full_attn) in enumerate(
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        block_klass(
                            dim_out + skip_connect_dim, dim_out, time_emb_dim=time_dim
                        ),
                        block_klass(
                            dim_out + skip_connect_dim, dim_out, time_emb_dim=time_dim
                        ),
...
class MedSegDiff(nn.Module):
    ...
    def p_losses(self, x_start, t, cond, noise=None):
        ...
        if self.objective == "pred_noise":
            target = noise
            pred_x0 = self.predict_start_from_noise(x, t, noise)
        elif self.objective == "pred_x0":
            target = x_start
            pred_x0 = x_start
        elif self.objective == "pred_v":
            ...
            target = v
            pred_x0 = self.predict_start_from_v(x, t, v)
        else:
            raise ValueError(f"unknown objective {self.objective}")

        return F.mse_loss(model_out, target), pred_x0
