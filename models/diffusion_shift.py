...
class ShiftDiffusion(nn.Module):
    ...
    def p_mean(self,
        ...
        if condition_x is not None:
            if model_out is None:
                model_out = model_forward(x_t, t, condition_x, self_cond)
        ...
        return model_mean,posterior_variance,posterior_log_variance_clipped,x_recon
    ...
    def q_sample(self, x_start,e_0, t, noise=None):
        ...
        return extract(self.etas, t, x_start.shape) * e_0 + x_start + extract(self.sqrt_kappa*self.etas, t,
                                                                                                x_start.shape) * noise


    def p_losses(self, x_start,y, noise=None, cond=None):
        '''
        :param x_start: HR
        :param y: LR
        :param noise:
        ...
        # q sample x_noisy
        e_0 = y - x_start #[b,24,h,w]
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, e_0=e_0,t=t, noise=noise)  # x_t
            ...
            with torch.no_grad():
                if not self.conditional:
                    ...
                    model_out = self.model(
                        x_noisy, t, cond=cond, self_cond=None
                    )
                if self.pred_mode == "noise":
                    x_self_cond = self.predict_start_from_noise(x_noisy, t, e_0,model_out)
                elif self.pred_mode == 'x_start':
        ...
        if not self.conditional:
            ...
            model_predict = self.model(
                x_noisy, t, cond=cond, self_cond=x_self_cond
            )
    ...
    def forward(self,x, y=None, cond=None,mode="train", *args, **kwargs):
        #e_0 = self.preprocess_encode_x(e_0)
        if mode == "train":
            return self.p_losses(x,y,cond=cond, *args, **kwargs)
        elif mode == "ddpm_sample":
            with torch.no_grad():
                return self.p_sample_loop(x,cond=cond, *args, **kwargs)
