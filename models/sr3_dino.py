...
class UNetSR3(nn.Module):
    def __init__(
        ...
        add_channel = int(in_channel if not self_condition else in_channel/2)
        self.add = nn.Sequential(
                Block(pre_channel,
                      add_channel,
                      groups=norm_groups),
                Upsample(add_channel)
        )
        ...
        self.final_attn = FastAttnCondInjection(add_channel,add_channel,pre_channel,out_channel,
            groups=8,
            nheads=8,
            drop_path_prob=0.2)
    ...
    def forward(self, x, time, cond=None, self_cond=None):
        ...
        pixel_emb = x_in+F.interpolate(addition,size=x_in.shape[-2:],mode="bilinear")
        dec_q = self.final_conv(x)  # + res
        out = self.final_attn(dec_q,pixel_emb)
        return out
...
class ResnetBlocWithAttn(nn.Module):
    def __init__(
        ...
        *,
        ...
        if with_attn:
            self.attn = SelfAttention(dim_out, norm_groups=norm_groups, n_head=4)
        if self.with_cond:
            if encoder:
